...
// Temporary route to fix location status discrepancies
router.post('/fix-location-statuses', authenticateToken, async (req, res) => {
  try {
    if (!req.user.isAdmin) {
      return res.status(403).json({ error: 'Admin access required' });
    }

    console.log('🔧 Starting location status fix...');
    ...
    const response = {
      success: true,
      message: 'Location status fix completed',
      summary: {
        totalLocations: allLocations.length,
        updatedCount,
        finalStats,
        verification: {
          locationsWith5PlusUpvotes: actuallyVerified,
          locationsMarkedAsVerified: finalStats.verified,
          discrepancyFixed: actuallyVerified === finalStats.verified
        }
      }
    };

    console.log(`✅ Status fix completed! Updated ${updatedCount} locations`);
    console.log(`🔍 Verification: ${actuallyVerified} locations with 5+ upvotes, ${finalStats.verified} marked as verified`);
    ...
    res.status(500).json({ 
      success: false, 
      error: 'Error fixing location statuses',
      details: error.message 
    });
  }
});
